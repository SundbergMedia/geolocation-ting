...
function sendResult(data, res, title) {
  if(!title) title = 'Geolocation ting';
  log('DATA:', data);
  res.render('template', { title: title, data: data }) // render HTML template
}
...
app.get('/ip/:ip', function (req, res) {
  ...
  helpers.getIpData(ip).then(function(data) {
    data.input = ip;
    sendResult(data, res, 'Geolocation ting - ' + ip);
  });
});
